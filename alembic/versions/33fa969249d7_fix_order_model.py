...
def upgrade() -> None:
    op.create_table('courier_applications',
    sa.Column('id', sa.BigInteger(), nullable=False),        # ← String() → BigInteger()
    sa.Column('user_id', sa.BigInteger(), nullable=False),   # ← String() → BigInteger()
    ...
    op.alter_column('orders', 'status',
               existing_type=postgresql.ENUM('CREATED', 'ACCEPTED', 'COOKING', 'ON_THE_WAY', 'DELIVERED', 'CANCELED', name='orderstatus'),
               type_=sa.String(),
...
def downgrade() -> None:
    op.alter_column('orders', 'status',
               ...
               type_=postgresql.ENUM('CREATED', 'ACCEPTED', 'COOKING', 'ON_THE_WAY', 'DELIVERED', 'CANCELED', name='orderstatus'),
               existing_nullable=False,
               existing_server_default=sa.text("'CREATED'::orderstatus"))
