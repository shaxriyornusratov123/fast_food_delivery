...
async def create_payment(session: db_dep, current_user: current_user_dep,create_data:PaymentCreateRequest):
    stmt=select(Order).where(Order.id==create_data.order_id, Order.user_id==current_user.id)
